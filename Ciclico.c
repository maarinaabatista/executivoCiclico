...
int main() {
    ...
    // Verificação do frame mínimo entre release-time e deadline de todos os jobs
    int frame_minimo = 2 * mdc;

    for (i = 0; i < n_tarefas; i++) {
        if (frame_minimo > tarefas[i].periodo) {
            frame_minimo = tarefas[i].periodo;
        }
    }

    // Verifica se o frame mínimo atende à condição especificada
    if (frame_minimo > 2 * mdc) {
        printf("A restrição de frame mínimo não é atendida para todas as tarefas.\n");
        return 1;
    }
